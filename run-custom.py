...
parser.add_argument("--a_d", type=float, default=0.05, help='driver step size (responsive: 1.0, lagging: 0.05)')
...
env_kwargs = {
    ...
    "a_d":              args.a_d,               # driver step: 1.0 = responsive, 0.05 = lagging
    "p_d":              1.,                     # Passengers always instantaneously responsive
...
class PPOAgent:
    ...
    def record_transition(self, state, next_state, reward, done):
...
if (torch.cuda.is_available()) and (args.use_cuda):
    agent_U.agent = agent_U.agent.cuda()
    agent_L.agent = agent_L.agent.cuda()
...
for n_epi in range(args.epochs):
    for t in range(agent_args.traj_length):
        ...
        agent_U.record_transition(state, next_state, reward_U, done)
        agent_L.record_transition(state, next_state, reward_L, done)

        # advance to next observation for the following step
        state_ = next_state_
        state = next_state
    ...
    state_ = observations[possible_agents[0]]
    # state = np.clip((state_ - state_rms.mean) / (state_rms.var ** 0.5 + 1e-8), -5, 5)
    state = state_
